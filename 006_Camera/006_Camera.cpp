...
int main()
{
	...
	GLFWwindow* win = glfwCreateWindow(WIDTH, HEIGHT, u8"FPS Camera(ADWS for move, Mouse for rotate and zoom)", NULL, NULL);
	if (win == NULL)
	{
	...
	while (!glfwWindowShouldClose(win))
	{
		...
		glm::mat4 view = cam.GetViewMatrix(); //glm::lookAt(camPos, camPos + camFront, camUp);
		...
		glm::mat4 projection = glm::perspective(glm::radians(cam.Fov), float(WIDTH) / float(HEIGHT), .1f, 100.f);
